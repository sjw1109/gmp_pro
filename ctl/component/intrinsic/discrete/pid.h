...
extern "C"
{
	...
	ctrl_gt ctl_step_pid_par(pid_regular_t* hpid, ctrl_gt input)
	{
		...
		hpid->sn = ctl_sat(hpid->sn + pwm_mul(input, (hpid->ki)),
			hpid->out_max, hpid->out_min);

		// output = P item + I item  + D item
		hpid->out = pwm_mul(input, hpid->kp)
			+ hpid->sn
			+ pwm_mul((input - hpid->dn), hpid->kd);
	...
	ctrl_gt ctl_step_pid_ser(pid_regular_t* hpid, ctrl_gt input)
	{
		...
		// I sum up
		// Bug fix: hpid->sn may overflow
		hpid->sn = ctl_sat(hpid->sn + pwm_mul(hpid->out, hpid->ki),
			hpid->out_max,hpid->out_min);

		// output += I item + D item
		hpid->out += hpid->sn + pwm_mul((input - hpid->dn), hpid->kd);
	...
	ec_gt ctl_setup_pid(pid_regular_t *hpid,
