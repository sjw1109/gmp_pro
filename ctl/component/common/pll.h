...
extern"C"
{
		...
		void ctl_step_pll(ctl_pll_t* pll)
	{
		...
		ctrl_gt error = -ctrl_mpy(pll->input.dat[0], current_phasor.dat[1])
			- ctrl_mpy(pll->input.dat[1], current_phasor.dat[0]);

		ctl_step_pid_ser(&pll->pid, error);

		ctrl_gt spd_filter = ctl_step_lowpass_filter(&pll->filter, pll->pid.out);

		pll->output_theta = pll->output_theta
			+ spd_filter;
		...
		// scale for PLL speed
		pll->output_freq = ctrl_mpy(spd_filter, pll->freq_sf);
	}
