...
void ctl_init()
{
    ...
    pmsm_ctrl_init.spd_pid_gain = 1.0f;
    ...
    pmsm_ctrl_init.acc_limit_min = -0.1f;
    pmsm_ctrl_init.acc_limit_max = 0.1f;
    ...
    ctl_pmsm_ctrl_current_mode(&pmsm_ctrl);
    ctl_set_pmsm_ctrl_idq_ff(&pmsm_ctrl, float2ctrl(0.2), float2ctrl(0.2));
